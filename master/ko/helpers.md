...
Laravel에는 다양한 전역 "헬퍼" PHP 함수가 포함되어 있습니다. 이들 함수 중 다수는 프레임워크 자체에서 사용되지만, 편리하게 느껴진다면 여러분의 애플리케이션에서도 자유롭게 사용할 수 있습니다.
...
<!-- 이하의 각 메서드별 상세 설명은 동일한 구조로 진행됩니다. 각 설명 내 영어 원문이 있는 경우 번역 텍스트만 제공(본문 예시는 생략) -->

<!-- 이후 항목들은 위 준수 규칙에 따라 번역이 수행됩니다. 아래는 각 세부 섹션 예시이며, 마크다운 구조와 표현을 반드시 유지하되, 코드‧키워드 등 부 번역 제외 항목은 그대로 두고, 기술적 흐름은 자연스럽게 풀어서 설명합니다. -->

<!-- 번역의 나머지는 위와 같은 규칙에 따라 이어서 작성됩니다 -->