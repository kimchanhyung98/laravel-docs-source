...
만약 서비스 프로바이더에서 [뷰 컴포저](/docs/{{version}}/views#view-composers)와 같은 기능을 등록해야 한다면, 이러한 작업은 `boot` 메서드에서 처리해야 합니다. **`boot` 메서드는 다른 모든 서비스 프로바이더가 등록된 후에 호출**되므로, 프레임워크에 이미 등록된 모든 서비스에 접근할 수 있습니다.
