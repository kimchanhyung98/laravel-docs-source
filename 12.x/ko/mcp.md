# Laravel MCP (Laravel MCP)

- [소개](#introduction)
- [설치](#installation)
    - [라우트 파일 퍼블리싱](#publishing-routes)
- [서버 생성](#creating-servers)
    - [서버 등록](#server-registration)
    - [웹 서버](#web-servers)
    - [로컬 서버](#local-servers)
- [툴](#tools)
    - [툴 생성](#creating-tools)
    - [툴 입력 스키마](#tool-input-schemas)
    - [툴 인수 유효성 검증](#validating-tool-arguments)
    - [툴 의존성 주입](#tool-dependency-injection)
    - [툴 애너테이션](#tool-annotations)
    - [툴 조건부 등록](#conditional-tool-registration)
    - [툴 응답](#tool-responses)
- [프롬프트](#prompts)
    - [프롬프트 생성](#creating-prompts)
    - [프롬프트 인수](#prompt-arguments)
    - [프롬프트 인수 유효성 검증](#validating-prompt-arguments)
    - [프롬프트 의존성 주입](#prompt-dependency-injection)
    - [프롬프트 조건부 등록](#conditional-prompt-registration)
    - [프롬프트 응답](#prompt-responses)
- [리소스](#creating-resources)
    - [리소스 생성](#creating-resources)
    - [리소스 URI 및 MIME 타입](#resource-uri-and-mime-type)
    - [리소스 요청](#resource-request)
    - [리소스 의존성 주입](#resource-dependency-injection)
    - [리소스 조건부 등록](#conditional-resource-registration)
    - [리소스 응답](#resource-responses)
- [인증](#authentication)
    - [OAuth 2.1](#oauth)
    - [Sanctum](#sanctum)
- [인가](#authorization)
- [서버 테스트](#testing-servers)
    - [MCP 인스펙터](#mcp-inspector)
    - [유닛 테스트](#unit-tests)

<a name="introduction"></a>
## 소개

[Laravel MCP](https://github.com/laravel/mcp)는 인공지능(AI) 클라이언트가 [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro)을 통해 Laravel 애플리케이션과 상호작용할 수 있도록 간단하고 우아한 방법을 제공합니다. 이 패키지는 서버, 툴, 리소스, 프롬프트를 선언적이고 직관적으로 정의할 수 있는 유창한(Fluent) 인터페이스를 제공하여, AI 기반 상호작용을 손쉽게 구축할 수 있도록 지원합니다.

<a name="installation"></a>
## 설치

Composer 패키지 매니저를 사용해 Laravel MCP를 프로젝트에 설치합니다.

```shell
composer require laravel/mcp
```

<a name="publishing-routes"></a>
### 라우트 파일 퍼블리싱

Laravel MCP 설치 후, MCP 서버를 정의할 수 있는 `routes/ai.php` 파일을 퍼블리시하려면 다음 Artisan 명령어를 실행하세요:

```shell
php artisan vendor:publish --tag=ai-routes
```

이 명령어는 애플리케이션의 `routes` 디렉터리에 `routes/ai.php` 파일을 생성합니다. 이 파일에서 MCP 서버를 등록할 수 있습니다.

<a name="creating-servers"></a>
## 서버 생성

MCP 서버는 `make:mcp-server` Artisan 명령어로 만들 수 있습니다. 서버는 MCP 기능(툴, 리소스, 프롬프트 등)을 AI 클라이언트에 노출하는 중심 통신 지점 역할을 합니다.

```shell
php artisan make:mcp-server WeatherServer
```

이 명령어는 `app/Mcp/Servers` 디렉터리에 새로운 서버 클래스를 생성합니다. 생성된 서버 클래스는 Laravel MCP의 기본 `Laravel\Mcp\Server` 클래스를 확장하며, 툴, 리소스, 프롬프트 등록을 위한 속성들을 제공합니다.

```php
<?php

namespace App\Mcp\Servers;

use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * 이 MCP 서버에 등록된 툴.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        // ExampleTool::class,
    ];

    /**
     * 이 MCP 서버에 등록된 리소스.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        // ExampleResource::class,
    ];

    /**
     * 이 MCP 서버에 등록된 프롬프트.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        // ExamplePrompt::class,
    ];
}
```

<a name="server-registration"></a>
### 서버 등록

서버를 생성한 후에는, 이를 `routes/ai.php` 파일에 등록해야 외부에서 접근할 수 있습니다. Laravel MCP는 서버 등록을 위해 `web`(HTTP 접속을 위한 서버) 및 `local`(명령행 서버) 두 가지 방법을 제공합니다.

<a name="web-servers"></a>
### 웹 서버

웹 서버(Web server)는 가장 일반적인 타입의 서버로, HTTP POST 요청을 통해 접근할 수 있습니다. 원격 AI 클라이언트나 웹 기반 통합에 이상적입니다. `web` 메서드로 웹 서버를 등록할 수 있습니다.

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/weather', WeatherServer::class);
```

일반 라우트와 마찬가지로, 미들웨어를 적용해 웹 서버를 보호할 수 있습니다.

```php
Mcp::web('/mcp/weather', WeatherServer::class)
    ->middleware(['throttle:mcp']);
```

<a name="local-servers"></a>
### 로컬 서버

로컬 서버(Local server)는 Artisan 명령어로 동작하며, [Laravel Boost](/docs/12.x/installation#installing-laravel-boost)와 같은 로컬 AI 비서 통합 구현에 적합합니다. `local` 메서드로 로컬 서버를 등록합니다.

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::local('weather', WeatherServer::class);
```

등록 후에는 보통 직접 `mcp:start` Artisan 명령어를 실행할 필요가 없습니다. 대신 MCP 클라이언트(AI 에이전트)가 서버를 시작하거나 [MCP 인스펙터](#mcp-inspector)를 사용할 수 있습니다.

<a name="tools"></a>
## 툴

툴은 서버가 AI 클라이언트에 호출 가능한 기능을 공개하도록 해줍니다. 툴을 통해 언어 모델이 특정 동작을 실행하거나, 코드를 실행하거나, 외부 시스템과 상호작용할 수 있습니다.

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\JsonSchema\JsonSchema;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴의 설명.
     */
    protected string $description = 'Fetches the current weather forecast for a specified location.';

    /**
     * 툴 요청 처리 메서드.
     */
    public function handle(Request $request): Response
    {
        $location = $request->get('location');

        // Get weather...

        return Response::text('The weather is...');
    }

    /**
     * 툴 입력 스키마 반환.
     *
     * @return array<string, \Illuminate\JsonSchema\JsonSchema>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),
        ];
    }
}
```

<a name="creating-tools"></a>
### 툴 생성

툴을 생성하려면 `make:mcp-tool` Artisan 명령어를 실행하세요.

```shell
php artisan make:mcp-tool CurrentWeatherTool
```

툴을 생성한 후에는, 서버의 `$tools` 속성에 등록해야 합니다.

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Tools\CurrentWeatherTool;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * 이 MCP 서버에 등록된 툴.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        CurrentWeatherTool::class,
    ];
}
```

<a name="tool-name-title-description"></a>
#### 툴 이름, 타이틀, 설명

기본적으로, 툴의 이름(name)과 타이틀(title)은 클래스명에서 자동으로 파생됩니다. 예를 들어, `CurrentWeatherTool` 클래스는 `current-weather`라는 이름과 `Current Weather Tool`이라는 타이틀을 갖습니다. 필요한 경우 `$name` 및 `$title` 속성을 정의해 직접 지정할 수 있습니다.

```php
class CurrentWeatherTool extends Tool
{
    /**
     * 툴 이름.
     */
    protected string $name = 'get-optimistic-weather';

    /**
     * 툴 타이틀.
     */
    protected string $title = 'Get Optimistic Weather Forecast';

    // ...
}
```

툴 설명(description)은 자동으로 생성되지 않으므로, 반드시 의미 있는 내용을 `$description` 속성에 직접 작성해야 합니다.

```php
class CurrentWeatherTool extends Tool
{
    /**
     * 툴 설명.
     */
    protected string $description = 'Fetches the current weather forecast for a specified location.';

    //
}
```

> [!NOTE]
> 설명(description)은 AI 모델이 툴을 효과적으로 사용해야 할 때와 방법을 이해하는 데 중요한 메타데이터입니다.

<a name="tool-input-schemas"></a>
### 툴 입력 스키마

툴은 AI 클라이언트로부터 어떤 인수를 받을지 입력 스키마로 명확히 지정할 수 있습니다. Laravel의 `Illuminate\JsonSchema\JsonSchema` 빌더를 사용해 입력 요구사항을 정의하세요.

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\JsonSchema\JsonSchema;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴 입력 스키마 반환.
     *
     * @return array<string, JsonSchema>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),

            'units' => $schema->enum(['celsius', 'fahrenheit'])
                ->description('The temperature units to use.')
                ->default('celsius'),
        ];
    }
}
```

<a name="validating-tool-arguments"></a>
### 툴 인수 유효성 검증

JSON Schema는 기본적인 툴 인수 구조만 정의해주므로, 더 복잡한 유효성 검증이 필요할 수도 있습니다.

Laravel MCP는 Laravel의 [유효성 검증 기능](/docs/12.x/validation)과 자연스럽게 통합됩니다. 툴의 `handle` 메서드에서 입력값에 대해 유효성 검증을 수행할 수 있습니다.

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴 요청 처리 메서드.
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'location' => 'required|string|max:100',
            'units' => 'in:celsius,fahrenheit',
        ]);

        // 검증된 인수로 날씨 데이터 조회...
    }
}
```

유효성 검증 실패 시, AI 클라이언트는 제공한 에러 메시지에 따라 동작합니다. 따라서 명확하고 실질적인 에러 메시지를 제공하는 것이 매우 중요합니다.

```php
$validated = $request->validate([
    'location' => ['required','string','max:100'],
    'units' => 'in:celsius,fahrenheit',
],[
    'location.required' => 'You must specify a location to get the weather for. For example, "New York City" or "Tokyo".',
    'units.in' => 'You must specify either "celsius" or "fahrenheit" for the units.',
]);
```

<a name="tool-dependency-injection"></a>
#### 툴 의존성 주입

Laravel의 [서비스 컨테이너](/docs/12.x/container)는 모든 MCP 툴을 해석(resolve)합니다. 따라서 툴의 생성자에서 필요한 의존성을 타입힌트하면, 컨테이너가 알아서 인스턴스를 주입해줍니다.

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 새 툴 인스턴스 생성자.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

생성자 주입 외에도, `handle()` 메서드의 인자에서도 의존성을 타입힌트할 수 있습니다. 서비스 컨테이너가 호출 시 자동으로 주입합니다.

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴 요청 처리 메서드.
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $location = $request->get('location');

        $forecast = $weather->getForecastFor($location);

        // ...
    }
}
```

<a name="tool-annotations"></a>
### 툴 애너테이션

[애너테이션](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations)을 활용해 툴의 추가 메타데이터를 AI 클라이언트에 제공할 수 있습니다. 애너테이션은 툴의 동작과 특성을 AI 모델이 더 잘 이해할 수 있도록 도와줍니다. 애너테이션은 속성(Attribute) 형태로 툴에 적용합니다.

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Server\Tools\Annotations\IsIdempotent;
use Laravel\Mcp\Server\Tools\Annotations\IsReadOnly;
use Laravel\Mcp\Server\Tool;

#[IsIdempotent]
#[IsReadOnly]
class CurrentWeatherTool extends Tool
{
    //
}
```

사용 가능한 애너테이션:

| 애너테이션             | 타입    | 설명                                                                                             |
| ---------------------- | ------- | ------------------------------------------------------------------------------------------------ |
| `#[IsReadOnly]`        | boolean | 툴이 환경을 변경하지 않음을 표시합니다.                                                         |
| `#[IsDestructive]`     | boolean | 툴이 파괴적인(destructive) 동작을 할 수 있음을 나타냅니다(읽기 전용이 아닐 때만 의미 있음).     |
| `#[IsIdempotent]`      | boolean | 동일 인수로 여러 번 호출해도 추가 효과가 없는 연산임을 표시합니다(읽기 전용이 아닐 때 의미 있음).|
| `#[IsOpenWorld]`       | boolean | 툴이 외부 엔터티와 상호작용할 수 있음을 나타냅니다.                                             |

<a name="conditional-tool-registration"></a>
### 툴 조건부 등록

툴 클래스에서 `shouldRegister` 메서드를 구현하여 런타임에 조건에 따라 툴을 등록할 수 있습니다. 애플리케이션의 상태, 설정, 요청 파라미터 기반으로 툴의 노출 여부를 제어할 수 있습니다.

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴을 등록해야 할지 여부를 결정.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

`shouldRegister`가 `false`를 반환하면, 해당 툴은 사용 가능한 목록에 나타나지 않으며 AI 클라이언트가 호출할 수 없습니다.

<a name="tool-responses"></a>
### 툴 응답

툴에서는 반드시 `Laravel\Mcp\Response` 인스턴스를 반환해야 합니다. Response 클래스는 다양한 유형의 응답을 쉽게 만들 수 있는 여러 메서드를 제공합니다.

간단한 텍스트 응답에는 `text` 메서드를 사용합니다.

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 툴 요청 처리 메서드.
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text('Weather Summary: Sunny, 72°F');
}
```

툴 실행 중 오류가 발생했음을 나타내려면 `error` 메서드를 사용하세요.

```php
return Response::error('Unable to fetch weather data. Please try again.');
```

<a name="multiple-content-responses"></a>
#### 다중 콘텐츠 응답

툴에서 `Response` 인스턴스의 배열을 반환하여 여러 개의 메시지를 순차적으로 제공할 수 있습니다.

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 툴 요청 처리 메서드.
 *
 * @return array<int, \Laravel\Mcp\Response>
 */
public function handle(Request $request): array
{
    // ...

    return [
        Response::text('Weather Summary: Sunny, 72°F'),
        Response::text('**Detailed Forecast**\n- Morning: 65°F\n- Afternoon: 78°F\n- Evening: 70°F')
    ];
}
```

<a name="streaming-responses"></a>
#### 스트리밍 응답

장시간 실행되거나 실시간 데이터가 필요한 경우, 툴에서 [generator](https://www.php.net/manual/en/language.generators.overview.php)를 반환해 중간 상태를 순차적으로 클라이언트에 전송할 수 있습니다.

```php
<?php

namespace App\Mcp\Tools;

use Generator;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴 요청 처리 메서드.
     *
     * @return \Generator<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): Generator
    {
        $locations = $request->array('locations');

        foreach ($locations as $index => $location) {
            yield Response::notification('processing/progress', [
                'current' => $index + 1,
                'total' => count($locations),
                'location' => $location,
            ]);

            yield Response::text($this->forecastFor($location));
        }
    }
}
```

웹 서버(Web 기반)의 경우, 스트리밍 응답이 자동으로 SSE(Server-Sent Events) 스트림을 열고 각 메시지를 클라이언트에 순차적으로 전달합니다.

<a name="prompts"></a>
## 프롬프트

[프롬프트(Prompts)](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts)는 서버가 AI 클라이언트에게 재사용이 가능한 프롬프트 템플릿을 제공할 수 있게 해줍니다. 이를 통해 공통 질의나 상호작용을 일관되게 구조화할 수 있습니다.

<a name="creating-prompts"></a>
### 프롬프트 생성

프롬프트를 만들려면 `make:mcp-prompt` Artisan 명령어를 실행하세요.

```shell
php artisan make:mcp-prompt DescribeWeatherPrompt
```

생성 후, 프롬프트 클래스를 서버의 `$prompts` 속성에 등록합니다.

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Prompts\DescribeWeatherPrompt;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * 이 MCP 서버에 등록된 프롬프트.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        DescribeWeatherPrompt::class,
    ];
}
```

<a name="prompt-name-title-and-description"></a>
#### 프롬프트 이름, 타이틀, 설명

기본적으로 프롬프트의 이름(name)과 타이틀(title)은 클래스명에서 파생됩니다. 예를 들어, `DescribeWeatherPrompt`는 `describe-weather`라는 이름과 `Describe Weather Prompt`라는 타이틀을 가집니다. 필요시 `$name`과 `$title`을 지정해 맞춤 설정할 수 있습니다.

```php
class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 이름.
     */
    protected string $name = 'weather-assistant';

    /**
     * 프롬프트 타이틀.
     */
    protected string $title = 'Weather Assistant Prompt';

    // ...
}
```

프롬프트 설명은 자동생성되지 않으므로, 알기 쉬운 내용을 직접 `$description`에 작성해야 합니다.

```php
class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 설명.
     */
    protected string $description = 'Generates a natural-language explanation of the weather for a given location.';

    //
}
```

> [!NOTE]
> 설명(description)은 AI 모델이 프롬프트를 언제, 어떻게 효과적으로 사용할 수 있는지 이해하는 데 중요한 메타데이터입니다.

<a name="prompt-arguments"></a>
### 프롬프트 인수

프롬프트는 외부에서 템플릿을 동적으로 구성할 수 있도록 인수(Arguments)를 정의할 수 있습니다. `arguments` 메서드를 사용해 허용하는 인수를 명확히 지정하세요.

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Server\Prompt;
use Laravel\Mcp\Server\Prompts\Argument;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 인수 반환.
     *
     * @return array<int, \Laravel\Mcp\Server\Prompts\Argument>
     */
    public function arguments(): array
    {
        return [
            new Argument(
                name: 'tone',
                description: 'The tone to use in the weather description (e.g., formal, casual, humorous).',
                required: true,
            ),
        ];
    }
}
```

<a name="validating-prompt-arguments"></a>
### 프롬프트 인수 유효성 검증

프롬프트 인수는 정의에 따라 자동으로 검증되지만, 더 복잡한 규칙이 필요할 경우가 있습니다.

Laravel MCP는 [유효성 검증 기능](/docs/12.x/validation)과 연동되므로, 프롬프트의 `handle` 메서드에서 인수 검증을 추가할 수 있습니다.

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 요청 처리 메서드.
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'tone' => 'required|string|max:50',
        ]);

        $tone = $validated['tone'];

        // tone 값에 맞춰 프롬프트 응답 생성...
    }
}
```

검증 실패 시 AI 클라이언트는 에러 메시지에 따라 동작하므로, 명확한 안내 메시지를 제공하는 것이 중요합니다.

```php
$validated = $request->validate([
    'tone' => ['required','string','max:50'],
],[
    'tone.*' => 'You must specify a tone for the weather description. Examples include "formal", "casual", or "humorous".',
]);
```

<a name="prompt-dependency-injection"></a>
### 프롬프트 의존성 주입

모든 프롬프트는 [서비스 컨테이너](/docs/12.x/container)에서 해석되어, 생성자 또는 `handle` 메서드의 타입힌트에 따라 의존성이 자동으로 주입됩니다.

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 새 프롬프트 인스턴스 생성자.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    //
}
```

`handle` 메서드에 타입힌트로 의존성을 선언하면, 서비스 컨테이너가 자동으로 주입해줍니다.

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 요청 처리 메서드.
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $isAvailable = $weather->isServiceAvailable();

        // ...
    }
}
```

<a name="conditional-prompt-registration"></a>
### 프롬프트 조건부 등록

프롬프트에서도 `shouldRegister` 메서드를 구현하여, 런타임 시 사용 가능 여부를 동적으로 제어할 수 있습니다.

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Prompt;

class CurrentWeatherPrompt extends Prompt
{
    /**
     * 프롬프트를 등록해야 할지 여부 결정.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

`shouldRegister`가 `false`를 반환하면, 프롬프트가 목록에 나타나지 않으며 호출할 수 없습니다.

<a name="prompt-responses"></a>
### 프롬프트 응답

프롬프트는 하나의 `Laravel\Mcp\Response` 또는 `Laravel\Mcp\Response` 인스턴스의 이터러블을 반환할 수 있습니다. 이 응답들은 AI 클라이언트에게 전송할 내용을 캡슐화합니다.

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 요청 처리 메서드.
     *
     * @return array<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): array
    {
        $tone = $request->string('tone');

        $systemMessage = "You are a helpful weather assistant. Please provide a weather description in a {$tone} tone.";

        $userMessage = "What is the current weather like in New York City?";

        return [
            Response::text($systemMessage)->asAssistant(),
            Response::text($userMessage),
        ];
    }
}
```

`asAssistant()` 메서드로 AI 어시스턴트가 작성한 메시지임을 명확하게 표시할 수 있습니다. 일반 메시지는 사용자 입력으로 간주됩니다.

<a name="resources"></a>
## 리소스

[리소스(Resources)](https://modelcontextprotocol.io/specification/2025-06-18/server/resources)는 서버가 AI 클라이언트에 읽을 수 있는 데이터 및 콘텐츠를 노출할 수 있게 해줍니다. AI가 동작을 결정하는 데 도움이 되는 문서, 설정, 각종 데이터를 공유할 수 있습니다.

<a name="creating-resources"></a>
## 리소스 생성

리소스를 생성하려면 `make:mcp-resource` Artisan 명령어를 사용하세요.

```shell
php artisan make:mcp-resource WeatherGuidelinesResource
```

생성 후, 서버의 `$resources` 속성에 등록합니다.

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Resources\WeatherGuidelinesResource;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * 이 MCP 서버에 등록된 리소스.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        WeatherGuidelinesResource::class,
    ];
}
```

<a name="resource-name-title-and-description"></a>
#### 리소스 이름, 타이틀, 설명

기본적으로 리소스의 이름과 타이틀은 클래스명에서 파생됩니다. 예시: `WeatherGuidelinesResource` → `weather-guidelines`, 타이틀: `Weather Guidelines Resource`. 필요시 `$name` 및 `$title` 속성으로 커스터마이징 가능합니다.

```php
class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스 이름.
     */
    protected string $name = 'weather-api-docs';

    /**
     * 리소스 타이틀.
     */
    protected string $title = 'Weather API Documentation';

    // ...
}
```

리소스 설명 역시 자동생성되지 않으니, 의미 있는 설명을 `$description`에 작성해야 합니다.

```php
class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스 설명.
     */
    protected string $description = 'Comprehensive guidelines for using the Weather API.';

    //
}
```

> [!NOTE]
> 설명(description)은 AI 모델에게 리소스가 언제, 어떻게 사용될 수 있는지 알려주는 중요한 메타데이터입니다.

<a name="resource-uri-and-mime-type"></a>
### 리소스 URI 및 MIME 타입

각 리소스는 고유한 URI와 MIME 타입을 갖습니다. 이를 통해 AI 클라이언트는 리소스 형식을 정확히 인식할 수 있습니다.

기본적으로 URI와 MIME 타입은 클래스명(이름)과 연결되어 자동 생성됩니다. 예를 들어, `WeatherGuidelinesResource`는 `weather://resources/weather-guidelines` URI와 `text/plain` MIME 타입을 가집니다. 필요시 `$uri`와 `$mimeType` 속성으로 값을 지정할 수 있습니다.

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스 URI.
     */
    protected string $uri = 'weather://resources/guidelines';

    /**
     * 리소스 MIME 타입.
     */
    protected string $mimeType = 'application/pdf';
}
```

URI와 MIME 타입은 AI 클라이언트가 리소스 콘텐츠를 올바르게 처리하고 해석하는 데 도움을 줍니다.

<a name="resource-request"></a>
### 리소스 요청

툴이나 프롬프트와 달리, 리소스는 입력 스키마나 인수를 정의할 수 없습니다. 하지만 `handle` 메서드에서 `Request` 객체로 요청 정보를 읽을 수 있습니다.

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스 요청 처리 메서드.
     */
    public function handle(Request $request): Response
    {
        // ...
    }
}
```

<a name="resource-dependency-injection"></a>
### 리소스 의존성 주입

모든 리소스는 [서비스 컨테이너](/docs/12.x/container)에서 해석되어, 생성자 및 `handle` 메서드 인자에 타입힌트한 의존성이 자동으로 주입됩니다.

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 새 리소스 인스턴스 생성자.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

또한, `handle` 메서드에 타입힌트를 선언하면 자동 주입됩니다.

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스 요청 처리 메서드.
     */
    public function handle(WeatherRepository $weather): Response
    {
        $guidelines = $weather->guidelines();

        return Response::text($guidelines);
    }
}
```

<a name="conditional-resource-registration"></a>
### 리소스 조건부 등록

리소스 클래스에서 `shouldRegister` 메서드를 구현하여 조건부 등록이 가능합니다. 앱의 상태, 설정, 요청 정보 등을 기반으로 동적으로 노출 여부를 결정할 수 있습니다.

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스를 등록해야 할지 여부 결정.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

`shouldRegister`가 `false`를 반환하면 목록에 표시되지 않으며, AI 클라이언트가 접근할 수 없습니다.

<a name="resource-responses"></a>
### 리소스 응답

리소스는 반드시 `Laravel\Mcp\Response` 인스턴스를 반환해야 합니다. 이 클래스는 다양한 응답 형식을 만들기 위한 편의 메서드를 제공합니다.

텍스트 콘텐츠는 `text` 메서드로 반환하세요.

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 리소스 요청 처리 메서드.
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text($weatherData);
}
```

<a name="resource-blob-responses"></a>
#### Blob 응답

Blob 데이터를 반환하려면 `blob` 메서드에 적절한 내용을 전달하세요.

```php
return Response::blob(file_get_contents(storage_path('weather/radar.png')));
```

Blob 응답의 MIME 타입은 리소스 클래스의 `$mimeType` 속성값을 기반으로 결정됩니다.

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스 MIME 타입.
     */
    protected string $mimeType = 'image/png';

    //
}
```

<a name="resource-error-responses"></a>
#### 에러 응답

리소스 조회 실패 등의 오류가 발생했을 때는 `error()` 메서드를 사용하세요.

```php
return Response::error('Unable to fetch weather data for the specified location.');
```

<a name="authentication"></a>
## 인증

웹 기반 MCP 서버는 일반 라우트와 마찬가지로 미들웨어를 통해 인증을 적용할 수 있습니다. 이를 적용하면 서버의 모든 기능을 사용하기 전에 인증이 요구됩니다.

MCP 서버를 보호하는 주요 방식은 두 가지가 있습니다: [Laravel Sanctum](/docs/12.x/sanctum)을 활용한 토큰 기반 인증과, `Authorization` HTTP 헤더에 API 토큰을 전달하는 방법, 그리고 [Laravel Passport](/docs/12.x/passport)를 이용한 OAuth 인증 방식입니다.

<a name="oauth"></a>
### OAuth 2.1

웹 기반 MCP 서버를 보호하는 가장 견고한 방법은 [Laravel Passport](/docs/12.x/passport)를 통한 OAuth 인증입니다.

OAuth를 통한 인증 사용 시, `routes/ai.php` 파일에서 `Mcp::oauthRoutes`를 호출해 필요한 OAuth2 디스커버리 및 클라이언트 등록 라우트를 구성합니다. 그리고, `Mcp::web` 라우트에 Passport의 `auth:api` 미들웨어를 적용하세요.

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::oauthRoutes();

Mcp::web('/mcp/weather', WeatherExample::class)
    ->middleware('auth:api');
```

#### Passport 신규 설치 시

Laravel Passport를 처음 도입하는 경우, [Passport 설치/배포 문서](/docs/12.x/passport#installation)를 따라 선행 설정 작업을 진행하세요. 즉, `OAuthenticatable` 모델, 인증 가드, passport 키를 준비해야 합니다.

그런 다음, Laravel MCP에서 제공하는 Passport 인가 뷰를 퍼블리시하세요.

```shell
php artisan vendor:publish --tag=mcp-views
```

그리고, `Passport::authorizationView` 메서드로 이 뷰를 등록해야 합니다. 일반적으로 `AppServiceProvider`의 `boot` 메서드에서 호출합니다.

```php
use Laravel\Passport\Passport;

/**
 * 애플리케이션 서비스 부트스트랩.
 */
public function boot(): void
{
    Passport::authorizationView(function ($parameters) {
        return view('mcp.authorize', $parameters);
    });
}
```

이 뷰는 인증 시 사용자가 AI 에이전트의 인증 요청을 승인 또는 거부할 때 표시됩니다.

![Authorization screen example](data:image/png;base64,)

> [!NOTE]
> 이 예시에서는 OAuth를 인증 가능(Authentiable)한 모델로의 변환 계층으로만 사용하며, OAuth의 범위(scope) 등 많은 특성은 무시합니다.

#### 기존 Passport 설치 활용

이미 Laravel Passport를 사용 중이라면, MCP는 별도 스코프 추가 없이(`mcp:use` 단일 스코프 사용) 현재 설치 환경에서 바로 동작합니다. 다만, 커스텀 스코프는 현재 지원되지 않습니다.

`Mcp::oauthRoutes()` 메서드가 위에서 말한 라우트를 등록하며, MCP는 OAuth를 인증 가능한 모델로의 변환 계층으로만 사용합니다.

#### Passport vs. Sanctum

OAuth 2.1은 Model Context Protocol 사양에 공식적으로 명시된 인증 방식이자, MCP 클라이언트에서 가장 널리 사용됩니다. 가능하다면 Passport를 사용하는 것을 권장합니다.

하지만 이미 [Sanctum](/docs/12.x/sanctum)을 사용 중이라면 Passport 도입이 부담스러울 수 있습니다. 이 경우 특별한 이유가 없다면 우선 Passport 없이 Sanctum만으로 MCP 서버 인증을 적용하시고, 필요성이 명확해진 후에만 Passport로 마이그레이션하는 것을 권장합니다.

<a name="sanctum"></a>
### Sanctum

[Sanctum](/docs/12.x/sanctum)으로 MCP 서버를 보호하려면, `routes/ai.php`에서 해당 서버 라우트에 Sanctum 인증 미들웨어를 추가하세요. MCP 클라이언트에서는 `Authorization: Bearer <token>` 헤더를 포함하여 인증을 완료해야 합니다.

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/demo', WeatherExample::class)
    ->middleware('auth:sanctum');
```

<a name="custom-mcp-authentication"></a>
#### 커스텀 MCP 인증

애플리케이션에서 자체적으로 API 토큰을 발급하는 경우, 원하는 미들웨어를 `Mcp::web` 라우트에 자유롭게 지정해 사용할 수 있습니다. 커스텀 미들웨어에서 `Authorization` 헤더를 직접 검사해 MCP 요청의 인증을 처리하면 됩니다.

<a name="authorization"></a>
## 인가

현재 인증된 사용자는 `$request->user()` 메서드로 접근할 수 있습니다. 이를 통해 MCP 툴과 리소스에서 [권한(Authorization) 체크](/docs/12.x/authorization)를 수행할 수 있습니다.

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 툴 요청 처리 메서드.
 */
public function handle(Request $request): Response
{
    if (! $request->user()->can('read-weather')) {
        return Response::error('Permission denied.');
    }

    // ...
}
```

<a name="testing-servers"></a>
## 서버 테스트

MCP 서버는 내장된 MCP 인스펙터 도구나, 유닛 테스트로 확인할 수 있습니다.

<a name="mcp-inspector"></a>
### MCP 인스펙터

[MCP 인스펙터](https://modelcontextprotocol.io/docs/tools/inspector)는 MCP 서버를 실시간으로 테스트하고 디버깅할 수 있는 인터랙티브 도구입니다. 서버에 연결해 인증 상태를 검증하거나 다양한 툴, 리소스, 프롬프트를 체험해볼 수 있습니다.

등록된 서버에 대해 MCP 인스펙터를 실행할 수 있습니다.

```shell
# 로컬 서버(예: "weather")...
php artisan mcp:inspector weather

# 웹 서버...
php artisan mcp:inspector /mcp/weather
```

이 명령어는 MCP 인스펙터를 실행하며, 연결에 사용할 클라이언트 설정을 안내합니다. 만약 인증 미들웨어로 보호된 서버라면, 연결 시 반드시 `Authorization` 베어러 토큰 등 요구되는 헤더를 제공해야 합니다.

<a name="unit-tests"></a>
### 유닛 테스트

MCP 서버, 툴, 리소스, 프롬프트에 대해 유닛 테스트를 작성할 수 있습니다.

테스트를 시작하려면 먼저 테스트 케이스를 만들고, 대상 서버에서 원시 프리미티브(툴, 프롬프트, 리소스 등)를 직접 호출합니다. 예를 들어, `WeatherServer`의 툴을 테스트하는 경우:

```php tab=Pest
test('tool', function () {
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
});
```

```php tab=PHPUnit
/**
 * 툴 테스트.
 */
public function test_tool(): void
{
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
}
```

마찬가지로 프롬프트와 리소스도 테스트할 수 있습니다.

```php
$response = WeatherServer::prompt(...);
$response = WeatherServer::resource(...);
```

인증된 사용자로 동작하려면, 프리미티브 실행 전에 `actingAs` 메서드를 체이닝할 수 있습니다.

```php
$response = WeatherServer::actingAs($user)->tool(...);
```

응답을 받은 후에는 다양한 Assertion 메서드로 결과와 상태를 검증할 수 있습니다.

성공 응답 여부는 `assertOk` 메서드로 확인합니다(오류가 없음을 검사).

```php
$response->assertOk();
```

특정 텍스트 포함 여부는 `assertSee`로 확인할 수 있습니다.

```php
$response->assertSee('The current weather in New York City is 72°F and sunny.');
```

오류 발생 여부는 `assertHasErrors`로, 특정 오류 메시지도 배열로 전달해 검사할 수 있습니다.

```php
$response->assertHasErrors();

$response->assertHasErrors([
    'Something went wrong.',
]);
```

오류가 없는 응답을 검사하려면 `assertHasNoErrors`를 사용합니다.

```php
$response->assertHasNoErrors();
```

응답 메타데이터(이름, 타이틀, 설명)도 각각 `assertName()`, `assertTitle()`, `assertDescription()` 메서드로 확인할 수 있습니다.

```php
$response->assertName('current-weather');
$response->assertTitle('Current Weather Tool');
$response->assertDescription('Fetches the current weather forecast for a specified location.');
```

노티피케이션 발송 여부는 `assertSentNotification`과 `assertNotificationCount`로 검사합니다.

```php
$response->assertSentNotification('processing/progress', [
    'step' => 1,
    'total' => 5,
]);

$response->assertSentNotification('processing/progress', [
    'step' => 2,
    'total' => 5,
]);

$response->assertNotificationCount(5);
```

마지막으로, 원시 응답 데이터를 직접 살펴보고 싶다면 `dd` 또는 `dump` 메서드로 값을 출력할 수 있습니다.

```php
$response->dd();
$response->dump();
```
