# Laravel MCP (Laravel MCP)

- [소개](#introduction)
- [설치](#installation)
    - [라우트 퍼블리싱](#publishing-routes)
- [서버 생성](#creating-servers)
    - [서버 등록](#server-registration)
    - [웹 서버](#web-servers)
    - [로컬 서버](#local-servers)
- [툴](#tools)
    - [툴 생성](#creating-tools)
    - [툴 입력 스키마](#tool-input-schemas)
    - [툴 인수 유효성 검증](#validating-tool-arguments)
    - [툴 의존성 주입](#tool-dependency-injection)
    - [툴 애노테이션](#tool-annotations)
    - [조건부 툴 등록](#conditional-tool-registration)
    - [툴 응답](#tool-responses)
- [프롬프트](#prompts)
    - [프롬프트 생성](#creating-prompts)
    - [프롬프트 인수](#prompt-arguments)
    - [프롬프트 인수 유효성 검증](#validating-prompt-arguments)
    - [프롬프트 의존성 주입](#prompt-dependency-injection)
    - [조건부 프롬프트 등록](#conditional-prompt-registration)
    - [프롬프트 응답](#prompt-responses)
- [리소스](#resources)
    - [리소스 생성](#creating-resources)
    - [리소스 URI 및 MIME 타입](#resource-uri-and-mime-type)
    - [리소스 요청](#resource-request)
    - [리소스 의존성 주입](#resource-dependency-injection)
    - [조건부 리소스 등록](#conditional-resource-registration)
    - [리소스 응답](#resource-responses)
- [인증](#authentication)
    - [OAuth 2.1](#oauth)
    - [Sanctum](#sanctum)
- [인가](#authorization)
- [서버 테스트](#testing-servers)
    - [MCP Inspector](#mcp-inspector)
    - [단위 테스트](#unit-tests)

<a name="introduction"></a>
## 소개 (Introduction)

[Laravel MCP](https://github.com/laravel/mcp)는 [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro)을 통해 AI 클라이언트가 여러분의 Laravel 애플리케이션과 상호작용할 수 있도록 간편하고 우아한 방식을 제공합니다. MCP는 서버, 툴, 리소스, 프롬프트를 정의할 수 있는 표현력 있고 직관적인 인터페이스를 제공하여, 애플리케이션에 AI 기반의 상호작용을 쉽게 탑재할 수 있게 해줍니다.

<a name="installation"></a>
## 설치 (Installation)

Composer 패키지 관리자를 사용해 프로젝트에 Laravel MCP를 설치하세요:

```shell
composer require laravel/mcp
```

<a name="publishing-routes"></a>
### 라우트 퍼블리싱 (Publishing Routes)

설치 이후, MCP 서버를 정의할 `routes/ai.php` 파일을 퍼블리싱하기 위해 다음 Artisan 명령어를 실행하세요:

```shell
php artisan vendor:publish --tag=ai-routes
```

이 명령은 애플리케이션의 `routes` 디렉토리에 `routes/ai.php` 파일을 생성하며, 이 파일에서 MCP 서버를 등록하게 됩니다.

<a name="creating-servers"></a>
## 서버 생성 (Creating Servers)

`make:mcp-server` Artisan 명령어를 사용하여 MCP 서버를 생성할 수 있습니다. 서버는 MCP의 중심 커뮤니케이션 포인트로, 툴, 리소스, 프롬프트 등 MCP 기능을 AI 클라이언트에 공개합니다:

```shell
php artisan make:mcp-server WeatherServer
```

이 명령은 `app/Mcp/Servers` 디렉토리에 새 서버 클래스를 생성합니다. 생성된 서버 클래스는 Laravel MCP의 기본 `Laravel\Mcp\Server` 클래스를 확장하며, 툴, 리소스, 프롬프트 등록을 위한 속성을 제공합니다:

```php
<?php

namespace App\Mcp\Servers;

use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * MCP 서버의 이름
     */
    protected string $name = 'Weather Server';

    /**
     * MCP 서버 버전
     */
    protected string $version = '1.0.0';

    /**
     * LLM을 위한 MCP 서버의 안내문
     */
    protected string $instructions = 'This server provides weather information and forecasts.';

    /**
     * 이 MCP 서버에 등록된 툴.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        // GetCurrentWeatherTool::class,
    ];

    /**
     * 이 MCP 서버에 등록된 리소스.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        // WeatherGuidelinesResource::class,
    ];

    /**
     * 이 MCP 서버에 등록된 프롬프트.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        // DescribeWeatherPrompt::class,
    ];
}
```

<a name="server-registration"></a>
### 서버 등록 (Server Registration)

서버를 생성한 후에는, 이를 `routes/ai.php` 파일에 등록해야 AI 클라이언트가 접근할 수 있습니다. MCP 서버는 HTTP로 접근 가능한 `web` 방식과 명령어 기반의 `local` 방식, 두 가지 방법으로 등록할 수 있습니다.

<a name="web-servers"></a>
### 웹 서버 (Web Servers)

웹 서버는 HTTP POST 요청을 통해 접근 가능하며, 원격 AI 클라이언트나 웹 기반 연동에 적합합니다. `web` 메서드로 웹 서버를 등록하세요:

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/weather', WeatherServer::class);
```

일반 라우트와 마찬가지로 미들웨어를 적용해 웹 서버를 보호할 수 있습니다:

```php
Mcp::web('/mcp/weather', WeatherServer::class)
    ->middleware(['throttle:mcp']);
```

<a name="local-servers"></a>
### 로컬 서버 (Local Servers)

로컬 서버는 Artisan 명령어로 실행되며, [Laravel Boost](/docs/12.x/installation#installing-laravel-boost)와 같은 로컬 AI 어시스턴트 연동에 적합합니다. `local` 메서드를 이용해 로컬 서버를 등록할 수 있습니다:

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::local('weather', WeatherServer::class);
```

서버를 등록한 후에는 일반적으로 직접 `mcp:start` Artisan 명령을 실행할 필요가 없습니다. 대신 MCP 클라이언트(AI 에이전트)가 서버를 시작하도록 하거나, [MCP Inspector](#mcp-inspector)를 사용할 수 있습니다.

<a name="tools"></a>
## 툴 (Tools)

툴은 MCP 서버가 외부에 기능을 공개할 수 있는 수단입니다. 이를 통해 AI 모델은 특정 작업을 실행하거나 코드를 실행하거나 외부 시스템과 상호작용할 수 있습니다:

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\JsonSchema\JsonSchema;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴 설명
     */
    protected string $description = 'Fetches the current weather forecast for a specified location.';

    /**
     * 툴 요청 처리
     */
    public function handle(Request $request): Response
    {
        $location = $request->get('location');

        // Get weather...

        return Response::text('The weather is...');
    }

    /**
     * 툴 입력 스키마 반환
     *
     * @return array<string, \Illuminate\JsonSchema\JsonSchema>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),
        ];
    }
}
```

<a name="creating-tools"></a>
### 툴 생성 (Creating Tools)

툴을 생성하려면 `make:mcp-tool` Artisan 명령어를 실행하세요:

```shell
php artisan make:mcp-tool CurrentWeatherTool
```

툴 생성 이후에는 서버의 `$tools` 속성에 등록해야 합니다:

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Tools\CurrentWeatherTool;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * MCP 서버에 등록된 툴
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        CurrentWeatherTool::class,
    ];
}
```

<a name="tool-name-title-description"></a>
#### 툴 이름, 타이틀 및 설명

기본적으로 툴의 이름(name)과 타이틀(title)은 클래스명에서 자동으로 생성됩니다. 예를 들어, `CurrentWeatherTool`의 이름은 `current-weather`, 타이틀은 `Current Weather Tool`이 됩니다. `$name` 및 `$title` 속성을 정의하여 직접 지정할 수도 있습니다:

```php
class CurrentWeatherTool extends Tool
{
    /**
     * 툴 이름
     */
    protected string $name = 'get-optimistic-weather';

    /**
     * 툴 타이틀
     */
    protected string $title = 'Get Optimistic Weather Forecast';

    // ...
}
```

툴 설명(description)은 자동으로 생성되지 않으므로, 항상 의미 있는 `$description` 속성을 정의해야 합니다:

```php
class CurrentWeatherTool extends Tool
{
    /**
     * 툴 설명
     */
    protected string $description = 'Fetches the current weather forecast for a specified location.';

    //
}
```

> [!NOTE]
> 설명은 툴의 메타데이터에서 매우 중요한 역할을 하며, AI 모델이 언제 어떤 방식으로 해당 툴을 효과적으로 사용할지 판단하는 데 큰 도움이 됩니다.

<a name="tool-input-schemas"></a>
### 툴 입력 스키마 (Tool Input Schemas)

툴은 AI 클라이언트가 전달하는 인수의 구조를 정의할 수 있습니다. Laravel의 `Illuminate\JsonSchema\JsonSchema` 빌더를 사용하여 툴의 입력 요구사항을 지정하세요:

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\JsonSchema\JsonSchema;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴 입력 스키마 반환
     *
     * @return array<string, JsonSchema>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),

            'units' => $schema->string()
                ->enum(['celsius', 'fahrenheit'])
                ->description('The temperature units to use.')
                ->default('celsius'),
        ];
    }
}
```

<a name="validating-tool-arguments"></a>
### 툴 인수 유효성 검증 (Validating Tool Arguments)

JSON 스키마는 툴 인수의 기본 구조만 지정해줍니다. 더 복잡한 유효성 검증이 필요한 경우, Laravel의 [유효성 검증 기능](/docs/12.x/validation)과 연동해 사용할 수 있습니다. 툴의 `handle` 메서드에서 인수 유효성 검증을 수행하세요:

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴 요청 처리
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'location' => 'required|string|max:100',
            'units' => 'in:celsius,fahrenheit',
        ]);

        // validated 인수로 날씨 데이터 조회...
    }
}
```

유효성 검증 실패 시, AI 클라이언트는 제공된 에러 메시지를 참고하므로, 명확하고 구체적인 메시지를 작성하는 것이 중요합니다:

```php
$validated = $request->validate([
    'location' => ['required','string','max:100'],
    'units' => 'in:celsius,fahrenheit',
],[
    'location.required' => 'You must specify a location to get the weather for. For example, "New York City" or "Tokyo".',
    'units.in' => 'You must specify either "celsius" or "fahrenheit" for the units.',
]);
```

<a name="tool-dependency-injection"></a>
#### 툴 의존성 주입 (Tool Dependency Injection)

Laravel의 [서비스 컨테이너](/docs/12.x/container)는 모든 MCP 툴 인스턴스를 해석할 때 사용되므로, 생성자에서 필요한 의존성을 타입힌트하면 자동으로 주입됩니다:

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 새 툴 인스턴스 생성
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

생성자 이외에도, 툴의 `handle()` 메서드에서도 의존성을 타입힌트할 수 있습니다. 서비스 컨테이너가 자동으로 주입합니다:

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴 요청 처리
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $location = $request->get('location');

        $forecast = $weather->getForecastFor($location);

        // ...
    }
}
```

<a name="tool-annotations"></a>
### 툴 애노테이션 (Tool Annotations)

툴에 [애노테이션](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations)을 추가하여 AI 클라이언트에 추가 메타데이터를 제공할 수 있습니다. 애노테이션을 활용하면 AI 모델이 툴의 동작과 특성을 보다 명확히 이해할 수 있습니다. 애노테이션은 PHP 속성(attribute)으로 부여합니다:

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Server\Tools\Annotations\IsIdempotent;
use Laravel\Mcp\Server\Tools\Annotations\IsReadOnly;
use Laravel\Mcp\Server\Tool;

#[IsIdempotent]
#[IsReadOnly]
class CurrentWeatherTool extends Tool
{
    //
}
```

사용 가능한 애노테이션 목록:

| 애노테이션         | 타입    | 설명                                                                                         |
| ------------------ | ------- | -------------------------------------------------------------------------------------------- |
| `#[IsReadOnly]`    | boolean | 툴이 환경을 수정하지 않음을 나타냄                                                           |
| `#[IsDestructive]` | boolean | 툴이 (read-only가 아닐 때) 파괴적 변경을 일으킬 수 있음을 나타냄                             |
| `#[IsIdempotent]`  | boolean | 동일 인수로 반복 호출해도 추가 효과가 없음 (read-only가 아닐 때만 의미를 가짐)                |
| `#[IsOpenWorld]`   | boolean | 툴이 외부 엔티티와 상호작용할 수 있음을 나타냄                                               |

<a name="conditional-tool-registration"></a>
### 조건부 툴 등록 (Conditional Tool Registration)

`shouldRegister` 메서드를 구현하면 애플리케이션 상태, 설정, 요청 정보 등에 따라 툴이 등록될지 동적으로 결정할 수 있습니다:

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴 등록 여부 결정
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

`shouldRegister`가 `false`를 반환하면 해당 툴은 사용 가능 목록에 나타나지 않으며, AI 클라이언트가 호출할 수 없습니다.

<a name="tool-responses"></a>
### 툴 응답 (Tool Responses)

툴은 반드시 `Laravel\Mcp\Response` 인스턴스를 반환해야 합니다. Response 클래스는 다양한 응답을 생성하기 위한 여러 메서드를 제공합니다.

간단한 텍스트 응답은 `text` 메서드로 생성합니다:

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 툴 요청 처리
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text('Weather Summary: Sunny, 72°F');
}
```

툴 실행 중 오류를 나타내려면 `error` 메서드를 사용합니다:

```php
return Response::error('Unable to fetch weather data. Please try again.');
```

<a name="multiple-content-responses"></a>
#### 다중 콘텐츠 응답

툴은 여러 개의 응답을 배열로 반환할 수 있습니다:

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 툴 요청 처리
 *
 * @return array<int, \Laravel\Mcp\Response>
 */
public function handle(Request $request): array
{
    // ...

    return [
        Response::text('Weather Summary: Sunny, 72°F'),
        Response::text('**Detailed Forecast**\n- Morning: 65°F\n- Afternoon: 78°F\n- Evening: 70°F')
    ];
}
```

<a name="streaming-responses"></a>
#### 스트리밍 응답 (Streaming Responses)

실행 시간이 길거나 실시간 데이터 스트리밍이 필요한 경우, `handle` 메서드에서 [generator](https://www.php.net/manual/en/language.generators.overview.php)를 반환할 수 있습니다. 각 응답은 AI 클라이언트로 단계별로 전송됩니다:

```php
<?php

namespace App\Mcp\Tools;

use Generator;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 툴 요청 처리
     *
     * @return \Generator<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): Generator
    {
        $locations = $request->array('locations');

        foreach ($locations as $index => $location) {
            yield Response::notification('processing/progress', [
                'current' => $index + 1,
                'total' => count($locations),
                'location' => $location,
            ]);

            yield Response::text($this->forecastFor($location));
        }
    }
}
```

웹 서버의 경우, 스트리밍 응답 사용 시 자동으로 SSE(Server-Sent Events) 스트림을 열어, 각 메시지를 이벤트로 실시간 전송합니다.

<a name="prompts"></a>
## 프롬프트 (Prompts)

[프롬프트](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts)는 서버가 AI 클라이언트에 재사용 가능한 프롬프트 템플릿을 제공하는 기능입니다. 자주 사용되는 쿼리와 상호작용을 표준화된 방식으로 구조화할 수 있습니다.

<a name="creating-prompts"></a>
### 프롬프트 생성 (Creating Prompts)

프롬프트를 생성하려면 `make:mcp-prompt` Artisan 명령어를 실행하세요:

```shell
php artisan make:mcp-prompt DescribeWeatherPrompt
```

생성한 프롬프트는 서버의 `$prompts` 속성에 등록해야 합니다:

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Prompts\DescribeWeatherPrompt;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * MCP 서버에 등록된 프롬프트
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        DescribeWeatherPrompt::class,
    ];
}
```

<a name="prompt-name-title-and-description"></a>
#### 프롬프트 이름, 타이틀 및 설명

기본적으로 프롬프트의 이름(name)과 타이틀(title)은 클래스명에서 자동으로 생성됩니다. 예를 들어, `DescribeWeatherPrompt`의 이름은 `describe-weather`, 타이틀은 `Describe Weather Prompt`가 됩니다. `$name`, `$title` 속성으로 직접 지정할 수도 있습니다:

```php
class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 이름
     */
    protected string $name = 'weather-assistant';

    /**
     * 프롬프트 타이틀
     */
    protected string $title = 'Weather Assistant Prompt';

    // ...
}
```

프롬프트 설명(description)도 직접 명시적으로 지정해야 합니다:

```php
class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 설명
     */
    protected string $description = 'Generates a natural-language explanation of the weather for a given location.';

    //
}
```

> [!NOTE]
> 설명은 프롬프트 메타데이터에서 매우 중요한 역할을 하며, AI 모델이 언제 어떻게 프롬프트를 최대한 활용할 수 있을지 이해하는 데 도움이 됩니다.

<a name="prompt-arguments"></a>
### 프롬프트 인수 (Prompt Arguments)

프롬프트는 AI 클라이언트가 특정 값을 전달하여 템플릿을 맞춤화하게 할 수 있습니다. `arguments` 메서드를 통해 프롬프트가 수락하는 인수를 정의하세요:

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Server\Prompt;
use Laravel\Mcp\Server\Prompts\Argument;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트의 인수 반환
     *
     * @return array<int, \Laravel\Mcp\Server\Prompts\Argument>
     */
    public function arguments(): array
    {
        return [
            new Argument(
                name: 'tone',
                description: 'The tone to use in the weather description (e.g., formal, casual, humorous).',
                required: true,
            ),
        ];
    }
}
```

<a name="validating-prompt-arguments"></a>
### 프롬프트 인수 유효성 검증 (Validating Prompt Arguments)

프롬프트 인수는 정의에 기반해 자동으로 기본 유효성 검증이 적용됩니다. 하지만 더 복잡한 검증이 필요하다면, Laravel의 [유효성 검증 기능](/docs/12.x/validation)을 그대로 사용할 수 있습니다:

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 요청 처리
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'tone' => 'required|string|max:50',
        ]);

        $tone = $validated['tone'];

        // tone에 따라 프롬프트 응답 생성...
    }
}
```

유효성 검증 실패 시 AI 클라이언트가 제공된 에러 메시지를 참고하므로, 신중히 작성하세요:

```php
$validated = $request->validate([
    'tone' => ['required','string','max:50'],
],[
    'tone.*' => 'You must specify a tone for the weather description. Examples include "formal", "casual", or "humorous".',
]);
```

<a name="prompt-dependency-injection"></a>
### 프롬프트 의존성 주입 (Prompt Dependency Injection)

모든 프롬프트는 Laravel [서비스 컨테이너](/docs/12.x/container)에서 해석됩니다. 따라서 생성자에 필요한 의존성을 타입힌트할 수 있으며 자동으로 인스턴스가 주입됩니다:

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 새 프롬프트 인스턴스 생성
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    //
}
```

또한 `handle` 메서드에서도 의존성을 타입힌트할 수 있고, 서비스 컨테이너가 알아서 인스턴스를 전달합니다:

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 요청 처리
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $isAvailable = $weather->isServiceAvailable();

        // ...
    }
}
```

<a name="conditional-prompt-registration"></a>
### 조건부 프롬프트 등록 (Conditional Prompt Registration)

프롬프트 클래스에서 `shouldRegister` 메서드를 구현하여 조건에 따라 프롬프트의 등록 여부를 동적으로 결정할 수 있습니다:

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Prompt;

class CurrentWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 등록 여부 결정
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

해당 메서드가 `false`를 반환할 경우, 프롬프트는 AI 클라이언트에 노출되지 않습니다.

<a name="prompt-responses"></a>
### 프롬프트 응답 (Prompt Responses)

프롬프트는 단일 `Laravel\Mcp\Response` 또는 여러 `Laravel\Mcp\Response` 인스턴스의 iterable(배열 등)을 반환할 수 있습니다. 각각은 AI 클라이언트에 전송될 메시지 내용을 담습니다:

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 프롬프트 요청 처리
     *
     * @return array<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): array
    {
        $tone = $request->string('tone');

        $systemMessage = "You are a helpful weather assistant. Please provide a weather description in a {$tone} tone.";

        $userMessage = "What is the current weather like in New York City?";

        return [
            Response::text($systemMessage)->asAssistant(),
            Response::text($userMessage),
        ];
    }
}
```

`asAssistant()` 메서드를 사용하면 해당 응답 메시지가 AI 어시스턴트의 응답임을 명확히 지정할 수 있습니다.

<a name="resources"></a>
## 리소스 (Resources)

[리소스](https://modelcontextprotocol.io/specification/2025-06-18/server/resources)는 서버가 AI 클라이언트에 참조 및 컨텍스트를 제공할 수 있도록 데이터를 노출하는 역할을 합니다. 문서, 설정, 각종 데이터 등 정적 또는 동적으로 AI 응답에 도움이 되는 정보를 제공할 수 있습니다.

<a name="creating-resources"></a>
## 리소스 생성 (Creating Resources)

리소스를 생성하려면 `make:mcp-resource` Artisan 명령어를 실행하세요:

```shell
php artisan make:mcp-resource WeatherGuidelinesResource
```

생성한 리소스는 서버의 `$resources` 속성에 등록합니다:

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Resources\WeatherGuidelinesResource;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * MCP 서버에 등록된 리소스
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        WeatherGuidelinesResource::class,
    ];
}
```

<a name="resource-name-title-and-description"></a>
#### 리소스 이름, 타이틀 및 설명

기본적으로 리소스의 이름(name)과 타이틀(title)은 클래스명에서 유추됩니다. 예를 들어, `WeatherGuidelinesResource`의 이름은 `weather-guidelines`, 타이틀은 `Weather Guidelines Resource`가 됩니다. `$name`, `$title` 속성으로 직접 지정할 수 있습니다:

```php
class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스 이름
     */
    protected string $name = 'weather-api-docs';

    /**
     * 리소스 타이틀
     */
    protected string $title = 'Weather API Documentation';

    // ...
}
```

리소스 설명(description)도 직접 제공해야 합니다:

```php
class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스 설명
     */
    protected string $description = 'Comprehensive guidelines for using the Weather API.';

    //
}
```

> [!NOTE]
> 설명은 리소스 메타데이터에서 중요한 부분으로, AI 모델이 리소스를 효과적으로 사용할 타이밍과 방법을 결정하는 데 도움이 됩니다.

<a name="resource-uri-and-mime-type"></a>
### 리소스 URI 및 MIME 타입 (Resource URI and MIME Type)

리소스마다 고유한 URI와 MIME 타입이 지정됩니다. MIME 타입은 AI 클라이언트가 해당 리소스의 포맷을 파악하는 데 사용합니다.

기본적으로 이름을 기반으로 URI가 생성되며, 예를 들어 `WeatherGuidelinesResource`는 `weather://resources/weather-guidelines`가 URI가 됩니다. 기본 MIME 타입은 `text/plain`입니다.

직접 `$uri`, `$mimeType` 속성을 지정할 수도 있습니다:

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스의 URI
     */
    protected string $uri = 'weather://resources/guidelines';

    /**
     * 리소스의 MIME 타입
     */
    protected string $mimeType = 'application/pdf';
}
```

이들 설정은 AI 클라이언트가 리소스 내용을 올바르게 해석할 수 있게 도와줍니다.

<a name="resource-request"></a>
### 리소스 요청 (Resource Request)

툴이나 프롬프트와 달리 리소스는 입력 스키마나 인수를 정의하지 않습니다. 하지만 `handle` 메서드에서 요청 객체와 상호작용하여 동적 처리를 할 수 있습니다:

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스 요청 처리
     */
    public function handle(Request $request): Response
    {
        // ...
    }
}
```

<a name="resource-dependency-injection"></a>
### 리소스 의존성 주입 (Resource Dependency Injection)

모든 리소스 클래스도 Laravel [서비스 컨테이너](/docs/12.x/container)를 통해 해석됩니다. 생성자/메서드에서 의존성 주입을 자유롭게 활용할 수 있습니다:

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 새 리소스 인스턴스 생성
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

`handle` 메서드에서 의존성을 타입힌트할 수도 있습니다:

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스 요청 처리
     */
    public function handle(WeatherRepository $weather): Response
    {
        $guidelines = $weather->guidelines();

        return Response::text($guidelines);
    }
}
```

<a name="conditional-resource-registration"></a>
### 조건부 리소스 등록 (Conditional Resource Registration)

리소스 클래스에서 `shouldRegister` 메서드를 구현하면 상황에 따라 리소스의 등록 여부를 결정할 수 있습니다:

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스 등록 여부 결정
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

`shouldRegister`가 `false`인 경우, 해당 리소스는 AI 클라이언트에 노출되지 않습니다.

<a name="resource-responses"></a>
### 리소스 응답 (Resource Responses)

리소스는 반드시 `Laravel\Mcp\Response` 인스턴스를 반환해야 합니다.

단순 텍스트 콘텐츠는 `text` 메서드로 응답할 수 있습니다:

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 리소스 요청 처리
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text($weatherData);
}
```

<a name="resource-blob-responses"></a>
#### Blob 응답

Blob(이진) 데이터 리소스의 경우, `blob` 메서드로 파일 등의 이진 내용을 응답할 수 있습니다:

```php
return Response::blob(file_get_contents(storage_path('weather/radar.png')));
```

이때 응답의 MIME 타입은 리소스 클래스의 `$mimeType` 속성 값에 따라 결정됩니다:

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 리소스의 MIME 타입
     */
    protected string $mimeType = 'image/png';

    //
}
```

<a name="resource-error-responses"></a>
#### 오류 응답

리소스 조회 중 오류 발생 시, `error()` 메서드를 사용하세요:

```php
return Response::error('Unable to fetch weather data for the specified location.');
```

<a name="authentication"></a>
## 인증 (Authentication)

일반적인 라우트와 마찬가지로, 웹 MCP 서버에도 미들웨어를 적용해 인증을 부여할 수 있습니다. 인증을 적용하면 MCP 서버의 모든 기능을 사용하기 전에 사용자가 반드시 인증을 거치게 됩니다.

MCP 서버 인증은 두 가지 방법이 있습니다: [Laravel Sanctum](/docs/12.x/sanctum)을 활용한 간단한 토큰 기반 인증, 또는 `Authorization` HTTP 헤더를 통한 임의 토큰 인증입니다. 아니면 [Laravel Passport](/docs/12.x/passport)를 통한 OAuth 인증을 사용할 수도 있습니다.

<a name="oauth"></a>
### OAuth 2.1

웹 기반 MCP 서버를 강력하게 보호하려면 [Laravel Passport](/docs/12.x/passport)를 통한 OAuth 방식을 권장합니다.

OAuth로 MCP 서버를 인증하려면, 먼저 `routes/ai.php` 파일에서 `Mcp::oauthRoutes` 메서드를 호출해 필요한 OAuth2 discovery 및 클라이언트 등록 라우트를 활성화하세요. 그 다음 `Mcp::web` 등록 시 Passport의 `auth:api` 미들웨어를 적용합니다:

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::oauthRoutes();

Mcp::web('/mcp/weather', WeatherExample::class)
    ->middleware('auth:api');
```

#### 새 Passport 설치

애플리케이션에 Laravel Passport가 아직 적용되어 있지 않다면, [Passport 설치 및 배포 가이드](/docs/12.x/passport#installation)를 참고하여 설치하세요. 설치 후에는 `OAuthenticatable` 모델, 인증 가드, 키 파일 등이 필요합니다.

다음으로, MCP에서 제공하는 Passport 인가 뷰도 퍼블리시합니다:

```shell
php artisan vendor:publish --tag=mcp-views
```

그리고 Passport의 `authorizationView` 메서드로 이 뷰를 사용하도록 지정하세요. 보통 `AppServiceProvider`의 `boot` 메서드에서 아래와 같이 등록합니다:

```php
use Laravel\Passport\Passport;

/**
 * 애플리케이션 서비스 부트스트랩
 */
public function boot(): void
{
    Passport::authorizationView(function ($parameters) {
        return view('mcp.authorize', $parameters);
    });
}
```

이 뷰는 최종 사용자가 AI 에이전트의 인증 시도를 승인 또는 거부할 수 있는 UI를 제공합니다.

![Authorization screen example](data:image/png;base64,)

> [!NOTE]
> 이 시나리오에서는 OAuth를 인증 가능한 모델로의 변환(translatation) 계층으로만 사용합니다. OAuth의 scope 등 다양한 측면은 무시되고 있습니다.

#### 기존 Passport 설치와 연동

이미 Laravel Passport를 사용하는 애플리케이션이라면, 추가 설정 없이 MCP와 바로 연동할 수 있습니다. 단, MCP에서는 OAuth의 커스텀 scope를 지원하지 않고, 오로지 하나의 `mcp:use` scope만 사용합니다.

#### Passport vs. Sanctum

OAuth 2.1은 Model Context Protocol에서 명세된 공식 인증 방식이며, MCP 클라이언트도 가장 널리 지원하는 인증 방식입니다. 따라서 특별한 사정이 없다면 Passport 사용이 권장됩니다.

이미 [Sanctum](/docs/12.x/sanctum)을 사용 중이고 Passport 추가가 부담된다면, 실제 필요에 따라 MCP 클라이언트가 OAuth만 지원해야 할 필요가 생기기 전까지는 Sanctum만 사용해도 됩니다.

<a name="sanctum"></a>
### Sanctum

[Sanctum](/docs/12.x/sanctum)을 이용해 MCP 서버를 보호하려면, `routes/ai.php`에서 Sanctum 인증 미들웨어를 추가하세요. MCP 클라이언트는 반드시 `Authorization: Bearer <token>` 헤더를 포함해야 인증이 완료됩니다:

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/demo', WeatherExample::class)
    ->middleware('auth:sanctum');
```

<a name="custom-mcp-authentication"></a>
#### 커스텀 MCP 인증 방식

애플리케이션에서 직접 API 토큰을 발급하는 경우, 원하는 미들웨어를 MCP 서버 라우트에 자유롭게 부여할 수 있습니다. 직접 구현한 미들웨어에서 `Authorization` 헤더 값을 참고해 인증 처리를 할 수 있습니다.

<a name="authorization"></a>
## 인가 (Authorization)

현재 인증된 사용자는 `$request->user()` 메서드로 접근할 수 있습니다. 이를 이용해 [인가(권한) 검사](/docs/12.x/authorization)를 MCP 툴 및 리소스에서 자유롭게 수행할 수 있습니다:

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 툴 요청 처리
 */
public function handle(Request $request): Response
{
    if (! $request->user()->can('read-weather')) {
        return Response::error('Permission denied.');
    }

    // ...
}
```

<a name="testing-servers"></a>
## 서버 테스트 (Testing Servers)

MCP Inspector 또는 단위 테스트를 통해 MCP 서버를 검증할 수 있습니다.

<a name="mcp-inspector"></a>
### MCP Inspector

[MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector)는 MCP 서버를 테스트하고 디버깅할 수 있는 대화형 도구입니다. Inspector를 사용해 서버 연결, 인증 확인, 툴·리소스·프롬프트 동작을 직접 확인할 수 있습니다.

등록된 서버에 대해 Inspector를 실행하려면 아래와 같이 명령어를 사용합니다:

```shell
# 웹 서버...
php artisan mcp:inspector mcp/weather

# "weather"라는 로컬 서버...
php artisan mcp:inspector weather
```

명령 실행 시 Inspector가 시작되며, MCP 클라이언트에 바로 적용할 수 있는 설정 정보도 출력합니다. 만약 인증 미들웨어로 보호된 서버라면, 반드시 인증 헤더(예: `Authorization` bearer 토큰)를 포함해야 정상 연결됩니다.

<a name="unit-tests"></a>
### 단위 테스트 (Unit Tests)

MCP 서버, 툴, 리소스, 프롬프트 각각에 대해 단위 테스트를 작성할 수 있습니다.

먼저 테스트 케이스를 만들고, 서버의 원하는 기능을 직접 호출하면 됩니다. 예를 들어, `WeatherServer`의 툴을 테스트하고 싶다면:

```php tab=Pest
test('tool', function () {
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
});
```

```php tab=PHPUnit
/**
 * 툴 테스트
 */
public function test_tool(): void
{
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
}
```

프롬프트와 리소스 테스트도 비슷하게 할 수 있습니다:

```php
$response = WeatherServer::prompt(...);
$response = WeatherServer::resource(...);
```

테스트 시 인증된 사용자로 동작하려면 `actingAs` 메서드를 체이닝하여 호출할 수 있습니다:

```php
$response = WeatherServer::actingAs($user)->tool(...);
```

응답을 받은 후에는 여러 assertion 메서드를 통해 결과와 상태를 검증할 수 있습니다.

성공적인 응답인지 확인하려면 `assertOk` 메서드를 사용하세요. 이는 응답에 에러가 없는지 검사합니다:

```php
$response->assertOk();
```

특정 텍스트가 응답에 포함되어 있는지 확인하려면 `assertSee`를 사용하세요:

```php
$response->assertSee('The current weather in New York City is 72°F and sunny.');
```

에러가 포함되어 있는지 검사하려면 `assertHasErrors` 메서드를 사용하세요:

```php
$response->assertHasErrors();

$response->assertHasErrors([
    'Something went wrong.',
]);
```

에러가 없는지 확인하려면 `assertHasNoErrors`를 사용하세요:

```php
$response->assertHasNoErrors();
```

응답의 메타데이터(이름, 타이틀, 설명 등)를 확인하고 싶다면 `assertName()`, `assertTitle()`, `assertDescription()` 메서드를 사용할 수 있습니다:

```php
$response->assertName('current-weather');
$response->assertTitle('Current Weather Tool');
$response->assertDescription('Fetches the current weather forecast for a specified location.');
```

알림(Notification)이 정상적으로 전송되었는지 확인하려면 `assertSentNotification` 및 `assertNotificationCount` 메서드를 사용합니다:

```php
$response->assertSentNotification('processing/progress', [
    'step' => 1,
    'total' => 5,
]);

$response->assertSentNotification('processing/progress', [
    'step' => 2,
    'total' => 5,
]);

$response->assertNotificationCount(5);
```

혹시 원시 응답 콘텐츠를 출력해 직접 디버깅하고 싶다면, `dd` 또는 `dump` 메서드를 사용할 수 있습니다:

```php
$response->dd();
$response->dump();
```
