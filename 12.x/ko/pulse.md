...
개별 이벤트의 상세한 디버깅이 필요하다면 [Laravel Telescope](/docs/12.x/telescope)도 참고하시기 바랍니다.
...
> [!NOTE]
> 인증된 사용자를 어떻게 수집하고 불러올지 완전히 커스터마이즈하려면, `Laravel\Pulse\Contracts\ResolvesUsers` 인터페이스를 구현한 클래스를 만들어 [서비스 컨테이너](/docs/12.x/container#binding-a-singleton)에 바인딩하면 됩니다.
...
`<livewire:pulse.slow-outgoing-requests />` 카드는 라라벨의 [HTTP 클라이언트](/docs/12.x/http-client)로 외부로 전송되는 요청 중, 1,000ms 이상의 임계값을 초과하는 요청을 보여줍니다.
...
> [!NOTE]
> Pulse는 재시작 신호를 [캐시](/docs/12.x/cache)에 저장하므로, 이 기능을 사용하려면 애플리케이션에 적절한 캐시 드라이버가 구성되어 있는지 확인해야 합니다.
...
`CacheInteractions` 레코더는 애플리케이션에서 발생하는 [캐시](/docs/12.x/cache) 적중 및 미스 정보를 [캐시 카드](#cache-card)에 표시할 수 있도록 저장합니다.
...
`SlowOutgoingRequests` 레코더는 라라벨의 [HTTP 클라이언트](/docs/12.x/http-client)로 외부로 전송한 요청 중, 설정된 임계값을 초과한 요청을 [느린 외부 요청 카드](#slow-outgoing-requests-card)에 표시할 수 있도록 캡처합니다.
...
Pulse는 기본적으로 [기본 Redis 커넥션](/docs/12.x/redis#configuration)을 사용하지만, `PULSE_REDIS_CONNECTION` 환경 변수로 커넥션을 지정할 수도 있습니다.
...
커스텀 카드가 애플리케이션 코드베이스 내에 위치해 있고, 라라벨의 [Vite 연동](/docs/12.x/vite)을 사용한다면, `vite.config.js` 파일에서 해당 카드의 CSS 엔트리포인트를 추가할 수 있습니다.
