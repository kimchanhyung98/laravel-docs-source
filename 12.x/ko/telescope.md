...
그리고 Telescope 패키지가 [자동 패키지 발견](/docs/12.x/packages#package-discovery)에 포함되지 않도록, 아래와 같이 여러분의 `composer.json` 파일에 설정을 추가해야 합니다.
...
데이터 정리를 하지 않으면 `telescope_entries` 테이블에 데이터가 빠르게 쌓일 수 있습니다. 이를 방지하기 위해 [스케줄러](/docs/12.x/scheduling)를 활용하여 `telescope:prune` 아티즌 명령어를 매일 실행하도록 예약하는 것이 좋습니다.
...
Telescope 대시보드는 `/telescope` 경로를 통해 접근할 수 있습니다. 기본적으로 `local` 환경에서만 접근이 허용됩니다. `app/Providers/TelescopeServiceProvider.php` 파일 안에는 [인가 게이트](/docs/12.x/authorization#gates) 정의가 포함되어 있으며, 이 게이트는 **비로컬 환경**에서의 Telescope 접근 권한을 제어합니다. 필요에 따라 이 게이트를 수정하고, Telescope 설치에 대한 접근 권한을 조정할 수 있습니다.
...
배치 Watcher는 큐에 등록된 [배치 작업](/docs/12.x/queues#job-batching)과 관련한 작업, 연결 정보 등을 기록합니다.
...
이벤트 Watcher는 애플리케이션에서 발생시킨 [이벤트](/docs/12.x/events)의 페이로드, 리스너, 브로드캐스트 데이터 등을 기록합니다. 라라벨 프레임워크 내부에서 발생하는 이벤트는 Watcher에서 자동으로 무시됩니다.
...
Gate Watcher는 애플리케이션에서 [Gate 및 Policy](/docs/12.x/authorization) 인가 검사를 수행할 때 해당 데이터와 결과를 기록합니다. 기록에서 제외하고 싶은 특정 ability가 있다면, `config/telescope.php` 파일의 `ignore_abilities` 옵션에 추가할 수 있습니다.
...
HTTP 클라이언트 Watcher는 애플리케이션에서 발생한 [HTTP 클라이언트 요청](/docs/12.x/http-client) 정보를 기록합니다.

<a name="job-watcher"></a>
### Job Watcher

Job Watcher는 애플리케이션에서 발생한 [큐 작업](/docs/12.x/queues)의 데이터 및 상태를 기록합니다.

<a name="log-watcher"></a>
### 로그 Watcher

로그 Watcher는 애플리케이션에서 기록된 [로그 데이터](/docs/12.x/logging)를 저장합니다.
...
메일 Watcher를 통해 애플리케이션에서 보낸 [이메일](/docs/12.x/mail)과 해당 데이터의 브라우저 내 미리보기가 가능합니다. 또한 메일을 `.eml` 파일로 다운로드할 수도 있습니다.

<a name="model-watcher"></a>
### 모델 Watcher

모델 Watcher는 Eloquent [모델 이벤트](/docs/12.x/eloquent#events)가 발생할 때 모델의 변경 사항을 기록합니다. 어떤 이벤트를 기록할지는 `events` 옵션에서 직접 지정할 수 있습니다.
...
알림 Watcher는 애플리케이션에서 전송한 [알림](/docs/12.x/notifications) 전체를 기록합니다. 만약 알림이 이메일을 트리거하고 메일 Watcher가 활성화된 경우, 해당 이메일도 메일 Watcher 화면에서 함께 확인할 수 있습니다.
...
Redis Watcher는 애플리케이션에서 수행한 모든 [Redis](/docs/12.x/redis) 명령을 기록합니다. Redis를 캐시 용도로 사용중이라면 캐시 관련 명령도 포함되어 기록됩니다.
...
스케줄 Watcher는 애플리케이션에서 실행한 [예약 작업](/docs/12.x/scheduling)의 명령어와 출력 결과를 기록합니다.

<a name="view-watcher"></a>
### 뷰 Watcher

뷰 Watcher는 [뷰](/docs/12.x/views)의 이름, 경로, 데이터, 뷰 컴포저 정보 등을 기록합니다.
